  ...
  const filtered = useMemo(() => {
    return nodes.filter((node) => {
      ...
      const matchedProvider =
        providerFilter === 'all' || node.provider_id === providerFilter || node.provider_name === providerFilter;
  ...
  return (
    <div>
      ...
      {filtered.length === 0 ? (
        ...
        <Card>
          <DataTable
            headers={['Name', 'Provider', 'Protocol', 'Server', 'Port', 'Region', 'Status', 'Last Seen', 'Actions']}
          >
            {filtered.map((node) => (
              <tr key={node.id} className="border-b border-slate-100 text-sm hover:bg-slate-50">
                <td className="px-4 py-3">{node.name}</td>
                <td className="px-4 py-3">{node.provider_name || node.provider_id}</td>
                ...
                <td className="px-4 py-3">{formatDateTime(node.last_seen_at)}</td>
                <td className="px-4 py-3">
                  <Button
